...
from models.modules import WaveletChannelEnhance3D
...
class CConv2(nn.Module):
    def __init__(self, channel):
        super(CConv2, self).__init__()
    ...
    def forward(self, float_fm, fixed_fm):
...
class PRC(nn.Module):
    def __init__(self, inshape=(160,192,160), flow_multiplier=1.,in_channel=1, channels=16):
        ...
        self.dconv4 = nn.Sequential(
            ConvInsBlock(2*8*c, 8*c),
            ConvInsBlock(8*c, 8*c)
        )
        2
        self.upconv3 = UpConvBlock(8*c, 4*c, 4, 2)
        self.cconv_3 = CConv2(2*4*c)

        # warp scale 1
        self.defconv3 = nn.Conv3d(2*4*c, 3, 3, 1, 1)
        ...
        self.dconv3 = ConvInsBlock(2 * 4 * c, 4 * c)
        
        self.upconv2 = UpConvBlock(2*4*c, 2*c, 4, 2)
        self.cconv_2 = CConv2(2*2*c)
        ...
        self.dconv2 = ConvInsBlock(2 * 2 * c, 2 * c)
        
        self.upconv1 = UpConvBlock(2*2*c, c, 4, 2)
        self.cconv_1 = CConv2(2*c)
        ...
        self.WHC4 = WaveletChannelEnhance3D(8*c)  # reduction = 16
    ...
    def forward(self, moving, fixed):
        ...
        F4_fre = self.WHC4(F4)
        # first dec layer
        C4 = torch.cat([M4_fre, F4_fre], dim=1)
        C4 = self.cconv_4(C4)  # (1,128,20,24,20)
        flow = self.defconv4(C4)  # (1,3,20,24,20)
        flow = self.diff[3](flow)
        warped4 = self.warp[3](M4, flow)



        flow = self.upsample_trilin(2*flow)
        warped3 = self.warp[2](M3, flow)  # (1, 64, 40, 48, 40)
        C3 = self.cconv_3(F3, warped3)  #  (1, 3 * 64, 40, 48, 40)
        v = self.defconv3(C3)
        w = self.diff[2](v)
        ...
        flow = self.upsample_trilin(2*flow)
        warped2 = self.warp[1](M2, flow)
        C2 = self.cconv_2(F2, warped2)
        v = self.defconv2(C2)  # (1,3,80,96,80)
        w = self.diff[1](v)
        flow = self.warp[1](flow, w)+w

        flow = self.upsample_trilin(2*flow)  # （1,3,160,196,160)
        warped1 = self.warp[0](M1, flow)  # （1,16,160,196,160)
        C1 = self.cconv_1(F1, warped1)  # （1,48,160,196,160)
        v = self.defconv1(C1)
        w = self.diff[0](v)
        flow = self.warp[0](flow, w)+w  # （1,3,160,196,160)

        warped1 = self.warp[0](M1, flow) 
        y_moved = self.warp[0](moving, flow)
        
  
        return y_moved, flow, warped1, F1, warped2, F2, warped3, F3, warped4, F4



if __name__ == '__main__':
    size = (1, 1, 160, 160, 128)
    model = PRC(size[2:]).cuda()
    # print(str(model))
    A = torch.ones(size).cuda()
    B = torch.ones(size).cuda()
    out, flow,_,_,_,_,_,_,_,_ = model(A, B)
