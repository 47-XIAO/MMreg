...
# 1. kernal size 2. muliply or addition
...
class RDP(nn.Module):
    def __init__(self, inshape=(160,192,160), flow_multiplier=1.,in_channel=1, channels=16):
        ...
        for i in range(4):
            ...
            self.attn.append(CMSA(F_g=c*(2**i), F_l=c*(2**i), F_int=c*(2**i)//2))
    ...
    def forward(self, moving, fixed):
        ...
        # first dec layer
        F4_s = self.attn[3](M4, F4)
        M4_s = self.attn[3](F4, M4)
        F4 = F4_s
        M4 = M4_s
        ...
        F3_s = self.attn[2](M3, F3)
        M3_s = self.attn[2](F3, M3)
        F3 = F3_s
        M3 = M3_s
        ...
        F2_s = self.attn[1](M2, F2)
        F2_s = self.attn[1](F2, M2)
        F2 = F2_s
        M2 = F2_s
        D2 = self.upconv2(C3)
        ...
        F1_s = self.attn[0](M1, F1)
        M1_s = self.attn[0](F1, M1)
        F1 = F1_s
        M1 = M1_s
