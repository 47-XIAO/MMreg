...
# from model import PR as RDP,PR
# from model import PR as network
...
def main():
    ...
    train_dir = '/hy-tmp/xsq/SR-Reg/train/volumes_center_norm_crop'
    val_dir = '/hy-tmp/xsq/SR-Reg/val/volumes_center_norm_crop'
    weights = [1, 1]  # loss weights
    lr = 1e-4
    save_dir = 'PRC_{}_reg_{}_lr_{}_sr_contl1_rampup_40e/'.format(*weights, lr)
    root = '/hy-tmp/xsq/RDP-main2/'
    if not os.path.exists('/hy-tmp/experiments_sr/' + save_dir):
        os.makedirs('/hy-tmp/experiments_sr/' + save_dir)
    if not os.path.exists('/hy-tmp/logs_sr/' + save_dir):
        os.makedirs('/hy-tmp/logs_sr/' + save_dir)
    sys.stdout = Logger('/hy-tmp/logs_sr/' + save_dir)
    ...
    for epoch in range(epoch_start, max_epoch + 1):
        ...
        }, save_dir='/hy-tmp/experiments_sr/' + save_dir, filename='dsc{:.3f}.pth.tar'.format(eval_dsc.avg))
        loss_all.reset()
